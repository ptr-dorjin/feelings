...
- `app-install.sh` installs the app on all virtual devices and leaves it there. Useful for
manually poking at a device; not a prerequisite for the next step, which installs fresh itself.

- `run-tests-on-all-devices.sh` installs the app fresh and runs UI tests on each virtual device
sequentially.
